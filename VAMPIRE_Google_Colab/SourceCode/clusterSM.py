...
def clusterSM(outpth, score, bdpc, clnum, pcnum=None, VamModel=None, BuildModel=None,
    ...
    if BuildModel:
        figdst = os.path.join(*[outpth, entries['Model name'], 'Example model figures'])
    else:
        figdst = os.path.join(outpth, 'Result based on ' + os.path.splitext(os.path.basename(entries['Model to apply']))[0])
